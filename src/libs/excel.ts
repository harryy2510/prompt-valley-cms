...
export function downloadTemplate(
	columns: Array<{ example?: string; header: string; key: string }>,
	filename: string,
	format: ExportFormat = 'xlsx'
) {
	const exampleRow: Record<string, string> = {}
	columns.forEach(({ example, header }) => {
	...
	// Auto-size columns
	const colWidths = columns.map(({ example, header }) => {
 ...
 */
export async function parseFile<T = Record<string, unknown>>(file: File): Promise<Array<T>> {
