 ...
 */
function parseMultiplePaths(input: string): string[] {
  return input
    .split(/[\n,]+/)
    .map((s) => s.trim())
    .filter((s) => s.length > 0)
  ...
  const handleFileChange = useCallback(
    async (file: File) => {
      ...
      if (filePath) {
        ...
        setImageItem((prev) =>
          prev
            ? { ...prev, status: 'error', error: 'Upload failed', progress: 0 }
            : null
        )
      }
    },
...
export function ImagesUpload({
  ...
  useEffect(() => {
    ...
    const removedPaths = [...internalPaths].filter(
      (path) => path && !externalPaths.has(path)
    )
      ...
      setImages((prev) => {
        ...
        const kept = prev.filter(
          (img) =>
            ...
            (img.bucketPath && externalPaths.has(img.bucketPath))
        )

        // Add new external items
        const newItems: ImageItem[] = newExternalPaths.map((path) => ({
          id: `external-${path}`,
  ...
  const handleFilesChange = useCallback(
    async (files: FileList | File[]) => {
      ...
      const successfulPaths = results.filter(
          ...
          {!showLinkInput ? (
            ...
            <div className="space-y-2 rounded-md border p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  Add{' '}
                  {parsedLinks.length > 1 ? `${parsedLinks.length} Links` : 'Link'}
                </Button>
                <Button
                  type="button"
                  size="sm"
